...
I have recently been enamoured with the idea of a fully declarative
home server. I have a general fear of forgetting configuration
details, made worse since each tool often has its own bespoke format.
Sometimes even the configuration files themselves are tucked away in
forgettable places, like `.env` files or certificates buried in
`/etc`. That said, I'm willing to concede that these are probably just
skill issues. In totality, it is the _ease of swapping hardware_ that
is ultimately the most persuasive reason to seutp a declarative
home-server.

Nix has been a resounding cure for this strange fear of mine. I can
specify my entire server in one language, from systemd services, to
project dependencies and deployment scripts. I recently learned
something cool about secrets and the Nix expression language's _lazy_
nature, so I wanted to share it here.
...
Then secrets started accumulating in project-specific directories,
like API keys, or into `/etc` for SSL certificates. Finally, the issue
compounded as my secrets began migrating to multiple machines. The
need for a more robust secret management scheme was clear.
...
on all my machines? It's probably best for machine-specific secrets to
be isolated. This is where the _lazy evaluation_ of Nix is perfect.
When building your configuration, `sops-nix` will only ever write out
the secret to a file if it is referenced in another attribute. In
other words, secrets are produced at runtime and on demand.
...
declarative dream is a beautiful one. This is my first blog post, so
