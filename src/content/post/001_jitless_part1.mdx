...
jumped into through the [EnterJIT Trampoline](links.enter_jit).
...
Firefox (see [V8's blog](links.v8_jitless) for a good introduction to
the concept.) In this post, we introduce the bootstrapping
...
symbol addresses into the [GOT](links.got) at load time. However, code
that references the GOT pays for an extra level of indirection on
...
for which we _don't_ want to emit AOT relocations. For that reason, we
implemented indirection for some runtime pointers, since cluttering
...
[BaselineFrame](links.baseline_frame) which facilitates the transition
between host C++ code and the JIT compiled baseline interpreter.
Immediately in the prologue we load the zone pointer into a
register<sup>1</sup>, then compute the runtime pointers through
