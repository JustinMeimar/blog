...
Using such an interface facilitates a much needed layer of abstraction
between describing the computation and the architectural specifics.
While no abstraction is perfectly leak-free (see some [quirks with
ARM](https://searchfox.org/firefox-main/rev/52e25e8bf7d712501f99b8ba77718ea0edc42bd7/js/src/jit/MacroAssembler.h#663)
as an illustrative example) overall, CPU instructions like `load`,
`store` and `add` are a perfectly reasonable interface to abstract.

Using a macro assembler, we only need to maintain the backends
responsible for correctly lowering the program, _not the program
itself_. Maintaining parallel interpreter implementations is quite
difficult, as V8 learned while [balancing several implementations at
once](https://v8.dev/blog/csa).
...
dependencies can generally be solved with
[bootstrapping](https://en.wikipedia.org/wiki/Bootstrapping_(compilers)).
However, if this were the only reason, the process to provide the
baseline interpreter AOT would be clear. We could simply generate the
...
were working on a vast majority of tests, once in a while stale jumps
were appearing. Why? It turns out, when `call(ImmPtr(target))` can't
reach the target with a 32-bit relative offset, the assembler sneakily
creates a trampoline at the bottom of the interpreter blob. We had to
patch these too, rewriting the `callWithABI(ImmPtr(fn))` with a
patchable `movabs` into a register followed by `callWithABI(reg)`.

```asm
;; Extended jump table entry rewritten through a register.
...
analysis tool to automate the detection of leaks.

On the other hand, our pointer patching technique bennefits from not
modifying the meticulous crafted codegen. We keep the emission
additional indirections to acheive position indpendence to a minimum.
