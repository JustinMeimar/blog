---
title: "Boostrapping an AOT Interpreter for SpiderMonkey [DRAFT]"
publishDate: 2026-02-03
description: "Patching Pointers to Supply a Runtime Generated Interpreter at Compile Time."
tags: ["cpp", "jit", "spidermonkey"]
---

You may have heard that high perofrmance interpreters are commonly [written in
assembly](). It may be surprising, however, to hear that SpiderMonkeys 
...
regular compiled obeject to be linked into the engine binary? To understand, lets take
a look at how assembler interfaces are designed. We will see how a runtime generated
interpreter has developer ergonomic as well as some performance bennefits. 

## Macro Assemblers

While high performance interpreter are etcheted out with the fine-grained precsion
assembly, the interpreter writer still finds convenience in using a producitve high
...
Using such an interface is not purely for developer ergonomics. It also facilliatates
...
No abstraction is perfectly leakless, with some [quirks with ARM]() for example.
...
emits it's threaded Baseline Interpreter.

> The term threaded means the interpreter
is organized as a series of op-code handlers, each associated a label. Rather than
...
Thus the visualization of a needle weaving a thread of execution flow through a sproadic
pattern of op handlers.

This demonstrates the ergonomics of emiting an interpreter through a C++ interface.
...
do with 8 implementations at once.) If we peek into some of the opcodes however, we
...
known symbol such as `Symbol.iterator` or `Symbol.toStringTag`, tyical utilities
for dynamic metaprogramming. In the first `movPtr` we observe the source of the
...
is non-determinsitic. Therefore, we may only know it's final address at runtime.
Since, in the sequence of temporal dependency, the generated interpreter comes last,
it can simply find the existing runtime and bake pointers to it's members in directly.
Great! This technique saves us from emiting position independent lookups. If the
interpreter could not generated for example, we may have needed to emit code to
...
While the generated interpreter has some desireable conveience and performance
...
ammortized over a long running process, for fast startup it is not ideal.
...
even replace the generic C++ interpreter as the inital tier. Additionally, as we will
talk about in pt.2, we could even supply a AOT corpus of statsitcally likely Inline Cache
stubs to further accelerate the baseline interpreter. But before we get there, we must
address the engineering challenge. How to make the baseline intepreter work as an
AOT compilation object without introducing slow, position independent code?

## Boostraping, Copying and Patching

The Copy-and-Patch technique is a familiar term to JIT compiler engineers, introduced
by Horan Xu & Freidric Koldjstad [1] in 2021 for performant baseline compilation.

> I mention C&P here as the primary source of inspiration for our technique. There
  are however, some differences. Friedric and Koldjstad generate a corpus op-handlers
  for a bytecode language dynamic using linker reloaction records to mark "holes" to patch
  ...
  for accessing a local variable. Check out their [paper]() for a much better explaination.

Our techinque is similar. As a thought experiment, if we write out the generated
...
our baseline compilation to emit AOT code. Rather than runtime addreesses, we bake in
...
### Boostrapping Infrastructure

Performing a build can be conceptualized as traversing a DAG where source nodes are source
files without dependencies. Compiling a node frees up one input dependency of it's consumer.
The optimal AOT boostrapping in this context looks like finding the minimal subgraph required
to spin up the engine and emit the baseline interpreter AOT blob, insert a new node into 
the dependency graph for the AOT interpreter, the continue on to complete the build.

In reality, this is too complex for me to program right now, nor am I sure how convential
